...
1. **Gather the input package** with the helper:
   ...
   ```bash
   node "$CLAUDE_PLUGIN_ROOT/lib/dream_prepare.js" --n ${ARGUMENTS:-25}
   ```
...
3. **Commit the rollup atomically** with the finalize helper:
   ...
   ```bash
   cat <<'CONTINUUM_DIGEST_EOF' | node "$CLAUDE_PLUGIN_ROOT/lib/dream_finalize.js" --rollup-ids "<comma-separated-ids>" --tags "phase-digest,<other-tags-from-tagsUnion>"
